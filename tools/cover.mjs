...
for (const n of levels) {
  await p.evaluate(i => __prism.load(i), n - 1); await p.waitForSelector('[data-a=p]');
  await p.evaluate(s => __prism.setStrokes(s), SOLUTIONS[n - 1]);
  const [x0, y0] = await p.evaluate(() => __prism.toScreen(0, 0)), [x1, y1] = await p.evaluate(() => __prism.toScreen(32, 18));
