...
## 24 Act 1 rethought: three orange lessons, then red; a HUD Skip (2026-09-06)

- **Asked for.** "The first 6 levels have this problem, most are just jump alone; we have dash as
  well, make use of it, rethink and design." Then: a Skip button in the HUD, definitely; and a
  Wavedash publish once done.
- **What the sim allows.** Red's bounce sets the normal speed to at least 27 u/s whatever the
  drop (docs/02), so a red pad tilted a hair is a full jump with a 13 u/s sideways carry — the
  same carry a dash-speed bounce gives. A level can therefore only *require* dash if it has no
  red ink at all, and a level with red ink is always solvable with red alone. Act 1 is now split
...
- **HUD Skip.** `Skip ›` in the top bar, offered when the next level would be open under the
  grid's rule: not on a level reached by skipping. It reuses the win screen's `nx` action. The
  condition needs no daily/race guard — generated levels have `li == LEVELS.length`.
...
- **Wavedash.** `dist/wavedash/index.html` is rebuilt from this source. The publish itself needs
  ...
  no user/machine variable); the two commands are in SUBMISSION.md.
