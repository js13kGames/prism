// 5-line static dev server: node dev.js [dir] → http://localhost:8080 (serves repo root or dir)
import http from 'http'; import fs from 'fs'; import path from 'path';
const T={html:'text/html',js:'text/javascript',css:'text/css',json:'application/json',png:'image/png'};
http.createServer((q,s)=>{const f=path.join(process.cwd(),process.argv[2]||'.',decodeURIComponent(q.url.split('?')[0].replace(/\/$/,'/index.html')));
